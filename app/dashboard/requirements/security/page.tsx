...
export default function SecurityGuardsPage() {
  ...
  const costPerGuard = 2500;
  const total = formData.quantity * formData.days * costPerGuard;
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border p-6 space-y-5">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500"
            placeholder="Any additional security requirements..."
          />
        </div>
        ...
        <div className="bg-amber-50 p-4 rounded-xl">
          ...
          <p className="text-xs text-gray-500 mt-1">@ ₹{costPerGuard.toLocaleString()} per guard per day</p>
        </div>

        <button
          type="submit"
          className="w-full py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-orange-600 transition"
        >
          Submit Requirement
        </button>
      </form>
