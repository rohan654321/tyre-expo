...
export interface CreateRentalItemData {
    ...
    name: string;
    description: string;
    costFor3Days: number;  // Make sure this matches
    category: 'AV' | 'IT' | 'Other';
    isActive: boolean;
    displayOrder?: number;
}
...
export async function getRentalItemStatistics() {
    const response = await api.get('/admin/rental-items/statistics');
    return response.data;
}